...
def left_down(e):
    e[0] == 'INPUT' and e[1].type == SDL_KEYDOWN and e[1].key == SDLK_LEFT
def left_up(e):
    e[0] == 'INPUT' and e[1].type == SDL_KEYUP and e[1].key == SDLK_LEFT
class Sleep:
    @staticmethod
    def enter(boy, e):
        boy.frame = 0
...
class Run:
    ...
    @staticmethod
    def do(boy):
        boy.frame = boy.frame(boy.frame + 1) % 8
        boy.x += boy.dir * 5
        pass
    @staticmethod
    def draw(boy):
...
class StateMactine:
    def __init__(self, boy):
        ...
        self.cur_state = Idle
        self.table = {
            Idle: {right_down: Run, left_down: Run, left_up: Run, left_down: Run, left_up: Run,
                   time_out: Sleep},  # time_out event를 발생시키지 않음
            Sleep: {right_down: Run, left_down: Run, left_up: Run, left_down: Run, left_up: Run,
                    space_down: Idle}, # Sleep에서 space_down일경우 Idle로 감 -> handle_event에서 변환해야 함
            Run: {right_down: Idle, left_down: Idle, right_up: Idle, left_down: Idle}
        }
        pass
    def start(self):
